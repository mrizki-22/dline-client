...
function Services() {
  ...
  async function fetchData() {
    try {
      const res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/services/`);
      setItems(res.data);
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <>
      <Navbar />
      <div className="flex flex-col font-poppins mt-5 mb-28 min-h-screen">
        ...
        <div className="grid gap-6 grid-cols-1 md:grid-cols-3 px-10 md:px-40 mt-20">
          {items ? (
            items.map((item, index) => {
              return (
                <div key={index} className=" flex flex-col justify-between bg-white px-10 py-12 rounded-[20px] drop-shadow-xl space-y-10 items-center">
                  <div className="box-content flex flex-col text-center w-[213px] items-center space-y-5">
                    ...
                    <p className="font-semibold text-[24px] text-space-cadet w-[300px]">{item.name}</p>
